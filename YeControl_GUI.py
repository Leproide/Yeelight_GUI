...
class YeelightManager(tk.Tk):
    def __init__(self):
        ...
        # Left panel (devices)
        frame_devices = tk.Frame(container, bd=2, relief=tk.GROOVE)
        frame_devices.grid(row=0, column=0, sticky="ns", padx=5, pady=5)
        
        tk.Label(frame_devices, text="Devices").pack(pady=(10, 0))
        self.listbox = tk.Listbox(frame_devices, width=30, height=20)
        self.listbox.pack(pady=5, padx=5)
        
        tk.Label(frame_devices, text="Name:").pack(anchor=tk.W, padx=5)
        self.entry_name = tk.Entry(frame_devices, width=25)
        self.entry_name.pack(pady=2, padx=5)
        
        tk.Label(frame_devices, text="IP:").pack(anchor=tk.W, padx=5)
        self.entry_ip = tk.Entry(frame_devices, width=25)
        self.entry_ip.pack(pady=2, padx=5)
        
        btn_add = tk.Button(frame_devices, text="Add", width=20, command=self.add_device)
        btn_add.pack(pady=5)
        btn_remove = tk.Button(frame_devices, text="Remove", width=20, command=self.remove_device)
        ...
        frame_controls = tk.Frame(container, bd=2, relief=tk.GROOVE)
        frame_controls.grid(row=0, column=1, sticky="nsew", padx=5, pady=5)
        container.columnconfigure(1, weight=1)
        
        frame_mode = tk.Frame(frame_controls)
        frame_mode.pack(pady=10)
        
        btn_night = tk.Button(frame_mode, text="Night Mode", width=20, command=self.night_mode)
        btn_night.pack(side=tk.LEFT, padx=5)
        
        btn_day = tk.Button(frame_mode, text="Day Mode", width=20, command=self.day_mode)
        btn_day.pack(side=tk.LEFT, padx=5)
        
        btn_toggle = tk.Button(frame_mode, text="Toggle On/Off", width=20, command=self.toggle_light)
        btn_toggle.pack(side=tk.LEFT, padx=5)
        
        # Sliders
        self.temp_slider = tk.Scale(frame_controls, from_=1700, to=6500, orient=tk.HORIZONTAL,
                                    label="Color Temperature", length=400)
        ...
        self.brightness_slider = tk.Scale(frame_controls, from_=1, to=100, orient=tk.HORIZONTAL,
                                          label="Brightness", length=400)
        self.brightness_slider.set(50)
        self.brightness_slider.pack(pady=10)
        self.brightness_slider.bind('<ButtonRelease-1>', self.on_brightness_slider_release)
        
        btn_color = tk.Button(frame_controls, text="Choose Color (dialog)", width=25, command=self.choose_color)
